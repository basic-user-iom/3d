...
/** Cache of tile URLs that returned 404/error so we don't re-request every frame and spam the console */
...
export default class WaterTileSource extends TileSource<Float32Array> {
	...
	private async fetchTile(x: number, y: number, zoom: number, scale: number): Promise<PBFPolygon[]> {
		...
		try {
			...
			failedTileUrls.add(url);
			// Return empty polygons - network/CORS failures are handled gracefully
		}
