...
const Config = {
	...
	// Use relative path when running on localhost (webpack proxy handles CORS)
	// Use absolute path for production builds
	// Note: This is evaluated at module load time, so it checks the main thread's window
	// Workers will receive this value via postMessage, so they'll use the correct template
	get TilesEndpointTemplate(): string {
		// Check if we're in a browser environment (not Node.js)
		if (typeof window === 'undefined') {
			// In worker context or Node.js - default to relative path (will be proxied)
			// The actual value will be passed from main thread
			return '/vector/{z}/{x}/{y}';
		}
		...
		const template = isLocalhost 
			? '/vector/{z}/{x}/{y}'
			: 'https://tiles.streets.gl/vector/{z}/{x}/{y}';
		
		// Only log once to avoid spam
		if (!(Config as any)._tilesEndpointLogged) {
			console.log('[Config] TilesEndpointTemplate:', template, { 
				isLocalhost, 
				hostname: window.location.hostname,
				port: window.location.port
			});
			(Config as any)._tilesEndpointLogged = true;
		}
