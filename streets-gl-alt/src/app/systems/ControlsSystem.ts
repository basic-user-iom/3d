...
export default class ControlsSystem extends System {
	...
	private pendingExternalState: { lat: number; lon: number; pitch: number; yaw: number; distance: number } | null = null;
	...
	public postInit(): void {
		...
		this.element = <HTMLCanvasElement>(this.systemManager.getCanvas() || document.getElementById('canvas'));
		
		if (!this.element) {
			console.error('[ControlsSystem] Cannot find canvas element');
			return;
		}
		...
		// Wait for SceneSystem to be ready before initializing camera
		this.systemManager.onSystemReady(SceneSystem, () => {
	...
	private initCameraAndNavigators(): void {
		...
		if (!sceneSystem || !sceneSystem.objects || !sceneSystem.objects.camera) {
			console.error('[ControlsSystem] SceneSystem or camera not ready');
			return;
		}
		...
		if (!terrainSystem || !terrainSystem.terrainHeightProvider) {
			console.error('[ControlsSystem] TerrainSystem not ready');
			return;
		}
		
		const terrainHeightProvider = terrainSystem.terrainHeightProvider;
	...
	private initStateFromHash(): void {
		...
		if (this.pendingExternalState) {
			const pending = this.pendingExternalState;
			this.pendingExternalState = null;
			this.setState(pending.lat, pending.lon, pending.pitch, pending.yaw, pending.distance);
		}
	}

	public getLatLon(): {lat: number; lon: number} {
		if (!this.camera || !this.camera.position) {
			// Return default position if camera not ready
			return { lat: 0, lon: 0 };
		}
	...
	public setState(lat: number, lon: number, pitch: number, yaw: number, distance: number): void {
		...
		if (!this.activeNavigator) {
			this.pendingExternalState = { lat, lon, pitch, yaw, distance };
			return;
		}
	...
	public update(deltaTime: number): void {
		if (!this.camera) {
			// Only initialize if SceneSystem is ready
			const sceneSystem = this.systemManager.getSystem(SceneSystem);
			if (sceneSystem && sceneSystem.objects && sceneSystem.objects.camera) {
				this.initCameraAndNavigators();
			} else {
				// SceneSystem not ready yet, skip this frame
				return;
			}
		}
