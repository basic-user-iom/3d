...
import ExternalObjectDepthMaterialContainer from "~/app/render/materials/ExternalObjectDepthMaterialContainer";
...
export default class ShadowMappingPass extends Pass<{
	...
	private readonly externalObjectMaterial: AbstractMaterial;
	...
	public constructor(manager: PassManager) {
		...
		this.externalObjectMaterial = new ExternalObjectDepthMaterialContainer(this.renderer).material;
	...
	private renderExternalObjects(shadowCamera: CSMCascadeCamera): void {
		...
		// Set up material uniforms (projection is same for all objects)
		...
		for (const object of externalObjects) {
			...
			// Frustum culling (optional, but recommended for performance)
			if (object.boundingBox && !object.inCameraFrustum(shadowCamera)) {
				continue;
			}
			...
			// Calculate model-view matrix (required by depth shader)
			const mvMatrix = Mat4.multiply(shadowCamera.matrixWorldInverse, object.matrixWorld);

			// Set per-object uniforms
			this.externalObjectMaterial.getUniform('modelViewMatrix', 'MainBlock').value =
				new Float32Array(mvMatrix.values);
			this.externalObjectMaterial.updateUniformBlock('MainBlock');

			// Draw the object for shadow mapping
			try {
				object.draw();
			} catch (error) {
				console.error(`[ShadowMappingPass] Error drawing external object ${(object as any).externalId || 'unknown'}:`, error);
			}
		}
	}

	private getExternalObjects(): ExternalRenderableObject[] {
		const objects: any[] = [this.manager.sceneSystem.scene];
	...
	public render(): void {
		...
		for (let i = 0; i < csm.cascadeCameras.length; i++) {
			...
			if (i < 2) {
				...
				this.renderExternalObjects(camera); // Render external objects for shadows
			}
