...
export default class GBufferPass extends Pass<{
	...
	private externalObjectDefaultBaseColorTexture: AbstractTexture2D;
	...
	private createMaterials(): void {
		...
		this.externalObjectDefaultBaseColorTexture =
			this.externalObjectMaterial.getUniform<UniformTexture2D>('tBaseColor').value as AbstractTexture2D;
	...
	private getExternalObjects(): ExternalRenderableObject[] {
		...
		// No per-frame logging when external objects exist - use ExternalObjectBridge logs for add/update instead
		return result;
	}
	...
	private renderExternalObjects(): void {
		...
		for (const object of externalObjects) {
			if ((object as any).visible === false) {
				continue
			}

			// Ensure matrixWorld is up to date (objects live under the floating-origin `wrapper`,
			// so matrixWorld is in the same near-origin render space as native geometry).
			object.updateMatrixWorld()

			// Lazily create mesh on first frame; draw in the same frame once ready.
			...
			// Frustum culling now works correctly because matrixWorld is wrapper-relative, exactly
			// like tiles and instanced objects. (No more skipFrustumCheck band-aid.)
			if (object.boundingBox && !object.inCameraFrustum(camera)) {
				continue;
			}
			...
			// Feed the object's real color into the shader (replaces hard-coded magenta debug color)
			const colorUniform = this.externalObjectMaterial.getUniform<UniformFloat3>('color', 'PerInstanceType');
			if (colorUniform) {
				colorUniform.value[0] = object.color.r;
				colorUniform.value[1] = object.color.g;
				colorUniform.value[2] = object.color.b;
			}

			const useBaseColorMapUniform = this.externalObjectMaterial.getUniform<UniformFloat1>('useBaseColorMap', 'PerInstanceType');
			if (useBaseColorMapUniform) {
				useBaseColorMapUniform.value[0] = object.useBaseColorMap && object.baseColorTexture ? 1.0 : 0.0;
			}

			const baseColorTextureUniform = this.externalObjectMaterial.getUniform<UniformTexture2D>('tBaseColor');
			if (baseColorTextureUniform) {
				baseColorTextureUniform.value = object.baseColorTexture ?? this.externalObjectDefaultBaseColorTexture;
			}

			// Write this object's picking id to the G-buffer object-id target so PickingSystem can
			// select it. 0 means "empty", so objects without an assigned id stay unpickable.
			const pickingId = (object as any).pickingId;
			const pickingIdUniform = this.externalObjectMaterial.getUniform<UniformInt1>('pickingId', 'PerInstanceType');
			if (pickingIdUniform) {
				pickingIdUniform.value[0] = typeof pickingId === 'number' ? pickingId : 0;
			}
