	...
	private async fetchTile(x: number, y: number, zoom: number): Promise<Record<string, PBFPolygon[]>> {
		...
		try {
			...
			if (response.status === 404) {
				// Tile doesn't exist - return empty polygons
				return polygons;
			}

			if (response.status !== 200) {
				// Try to parse error message, but return empty polygons anyway
				try {
					...
					// Error response is not JSON, just return empty polygons
				}
				return polygons;
			}
			...
			// Network errors or other fetch failures - return empty polygons
			// Missing tiles are normal for map systems
			return polygons;
		}
