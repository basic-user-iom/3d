...
import ExternalObjectMaterialContainer from "~/app/render/materials/ExternalObjectMaterialContainer";
import ExternalRenderableObject from "~/app/objects/ExternalRenderableObject";
...
export default class GBufferPass extends Pass<{
	...
	private externalObjectMaterial: AbstractMaterial;
	private cameraMatrixWorldInversePrev: Mat4 = null;
	private lastExternalObjectLog: number = 0;
	...
	private createMaterials(): void {
		...
		this.externalObjectMaterial = new ExternalObjectMaterialContainer(this.renderer).material;
		// Use instance texture array if available
		const instanceTexture = this.manager.texturePool.get('instance');
		if (instanceTexture) {
			this.externalObjectMaterial.getUniform<UniformTexture2DArray>('tMap').value =
				<AbstractTexture2DArray>instanceTexture;
		}
	}
	...
	/**
	 * Collect all ExternalRenderableObject instances from the scene
	 */
	private getExternalObjects(): ExternalRenderableObject[] {
		const objects: Object3D[] = [this.manager.sceneSystem.scene];
		const result: ExternalRenderableObject[] = [];
		...
		// Log only when we have external objects, throttled to every 2s (no "found 0" spam)
		const now = Date.now();
		if (result.length > 0 && (!this.lastExternalObjectLog || now - this.lastExternalObjectLog > 2000)) {
			console.log(`[GBufferPass] Scene traversal: checked ${totalObjectsChecked} objects, found ${result.length} external object(s)`);
			this.lastExternalObjectLog = now;
		}
	...
	/**
	 * Render external objects (cars, models from Three.js)
	 */
	private renderExternalObjects(): void {
		const camera = this.manager.sceneSystem.objects.camera;
		const externalObjects = this.getExternalObjects();
		...
		if (externalObjects.length > 0 && !this.lastExternalObjectLog || Date.now() - this.lastExternalObjectLog > 1000) {
			console.log(`[GBufferPass] Rendering ${externalObjects.length} external object(s)`)
			this.lastExternalObjectLog = Date.now()
		}

		this.renderer.useMaterial(this.externalObjectMaterial);

		// Set up material uniforms
		this.externalObjectMaterial.getUniform('projectionMatrix', 'MainBlock').value =
			new Float32Array(camera.jitteredProjectionMatrix.values);
		this.externalObjectMaterial.getUniform('viewMatrix', 'MainBlock').value =
			new Float32Array(camera.matrixWorldInverse.values);
		// CRITICAL: Update the uniform block after setting view/projection matrices
		this.externalObjectMaterial.updateUniformBlock('MainBlock');

		// Debug: Log all external objects and their frustum status (once per second)
		if (externalObjects.length > 0 && (!this.lastExternalObjectLog || Date.now() - this.lastExternalObjectLog > 1000)) {
			const cameraPos = { x: camera.position.x, y: camera.position.y, z: camera.position.z }
			const objectsInfo = externalObjects.map((obj: any) => ({
				id: obj.externalId || 'unknown',
				position: { x: obj.position.x, y: obj.position.y, z: obj.position.z },
				inFrustum: obj.inCameraFrustum(camera),
				meshReady: obj.isMeshReady(),
			...
			console.log(`[GBufferPass] External objects debug:`, {
				totalObjects: externalObjects.length,
				cameraPosition: cameraPos,
				objects: objectsInfo
			})
			
			// Also log as separate lines for better readability
			console.log(`[GBufferPass] Camera at: x=${cameraPos.x.toFixed(2)}, y=${cameraPos.y.toFixed(2)}, z=${cameraPos.z.toFixed(2)}`)
			objectsInfo.forEach((obj: any) => {
				console.log(`[GBufferPass] Object ${obj.id}: pos(${obj.position.x.toFixed(2)}, ${obj.position.y.toFixed(2)}, ${obj.position.z.toFixed(2)}), inFrustum=${obj.inFrustum}, distance=${obj.distanceFromCamera.toFixed(2)}m`)
			})
		...
		for (const object of externalObjects) {
			// Ensure matrixWorld is up to date before frustum culling
			object.updateMatrixWorld()
			
			// TEMPORARILY DISABLE FRUSTUM CULLING FOR DEBUGGING
			// Objects are being incorrectly culled even when they should be visible
			// TODO: Fix frustum culling calculation for objects at large world coordinates
			const skipFrustumCheck = true // Set to false to re-enable frustum culling
			
			if (!skipFrustumCheck && object.boundingBox) {
				const inFrustum = object.inCameraFrustum(camera)
				if (!inFrustum) {
					// Skip objects outside frustum (but log occasionally for debugging)
					if (externalObjects.length > 0 && (!this.lastExternalObjectLog || Date.now() - this.lastExternalObjectLog > 2000)) {
						const worldPos = object.position
						console.warn(`[GBufferPass] Object ${(object as any).externalId || 'unknown'} culled (not in frustum)`, {
							worldPosition: { x: worldPos.x, y: worldPos.y, z: worldPos.z },
							cameraPosition: { x: camera.position.x, y: camera.position.y, z: camera.position.z },
							hasBoundingBox: !!object.boundingBox,
							boundingBox: object.boundingBox ? {
								min: { x: object.boundingBox.min.x, y: object.boundingBox.min.y, z: object.boundingBox.min.z },
								max: { x: object.boundingBox.max.x, y: object.boundingBox.max.y, z: object.boundingBox.max.z }
							} : null
						})
					}
					continue
			...
			// Update object's mesh if needed
			if (!object.isMeshReady()) {
				object.updateMesh(this.renderer);
				continue;
			}
			
			// Debug: Log object position and camera position (throttled)
			if (externalObjects.length > 0 && (!this.lastExternalObjectLog || Date.now() - this.lastExternalObjectLog > 2000)) {
				const worldPos = object.position
				const distance = Math.sqrt(
					Math.pow(camera.position.x - worldPos.x, 2) +
					Math.pow(camera.position.y - worldPos.y, 2) +
					Math.pow(camera.position.z - worldPos.z, 2)
				)
				const scale = object.scale
				console.log(`[GBufferPass] 🔍 Rendering object ${(object as any).externalId || 'unknown'}:`, {
					...
					hasBoundingBox: !!object.boundingBox,
					matrixWorld: object.matrixWorld ? 'exists' : 'missing'
				})
				this.lastExternalObjectLog = Date.now()
			}

			// Calculate model-view matrix
			const mvMatrix = Mat4.multiply(camera.matrixWorldInverse, object.matrixWorld);
			const mvMatrixPrev = this.cameraMatrixWorldInversePrev
				? Mat4.multiply(this.cameraMatrixWorldInversePrev, object.matrixWorld)
				: mvMatrix;

			// Set per-object uniforms
			this.externalObjectMaterial.getUniform('modelMatrix', 'MainBlock').value =
				new Float32Array(object.matrixWorld.values);
			this.externalObjectMaterial.getUniform('modelViewMatrixPrev', 'MainBlock').value =
				new Float32Array(mvMatrixPrev.values);
			this.externalObjectMaterial.updateUniformBlock('MainBlock');

			// Set textureId (default to 0, can be customized per object via metadata)
			const metadata = (object as any).externalMetadata;
			const textureId = metadata && metadata.textureId !== undefined ? metadata.textureId : 0.0;
			const textureIdUniform = this.externalObjectMaterial.getUniform<UniformFloat1>('textureId', 'PerInstanceType');
			if (textureIdUniform) {
				textureIdUniform.value[0] = textureId;
				this.externalObjectMaterial.updateUniformBlock('PerInstanceType');
			}

			// Draw the object
			try {
				// Log every draw for debugging (remove throttling temporarily)
				const objId = (object as any).externalId || 'unknown'
				const worldPos = object.position
				const distance = Math.sqrt(
					Math.pow(camera.position.x - worldPos.x, 2) +
					Math.pow(camera.position.y - worldPos.y, 2) +
				...
				// Verify mesh exists before drawing
				if (!object.mesh) {
					console.error(`[GBufferPass] ❌ Cannot draw object ${objId}: mesh is null!`)
					continue
				}
				
				// Verify mesh has valid attributes
				const positionAttr = object.mesh.getAttribute('position')
				if (!positionAttr) {
					console.error(`[GBufferPass] ❌ Cannot draw object ${objId}: mesh has no position attribute!`)
					continue
				}
				
				console.log(`[GBufferPass] 🎬 Drawing object ${objId}: pos(${worldPos.x.toFixed(1)}, ${worldPos.y.toFixed(1)}, ${worldPos.z.toFixed(1)}), dist=${distance.toFixed(1)}m, scale=(${object.scale.x.toFixed(2)}, ${object.scale.y.toFixed(2)}, ${object.scale.z.toFixed(2)}), vertices=${positionAttr.buffer ? 'present' : 'missing'}`)
				
				// CRITICAL: Ensure material is bound before drawing
				// The material might have been unbound by previous draw calls
				this.renderer.useMaterial(this.externalObjectMaterial);
				
				// Verify material is bound BEFORE drawing
				const currentMaterial = (this.renderer as any).currentMaterial
				console.log(`[GBufferPass] Material check for ${objId} (BEFORE draw):`, {
					hasMaterial: !!this.externalObjectMaterial,
					currentMaterial: currentMaterial ? 'bound' : 'not bound',
					hasViewMatrix: !!this.externalObjectMaterial.getUniform('viewMatrix', 'MainBlock'),
					hasProjectionMatrix: !!this.externalObjectMaterial.getUniform('projectionMatrix', 'MainBlock'),
					hasModelMatrix: !!this.externalObjectMaterial.getUniform('modelMatrix', 'MainBlock')
				})
				
				object.draw();
				console.log(`[GBufferPass] ✅ Successfully drew object ${objId}`)
			} catch (error) {
				console.error(`[GBufferPass] ❌ Error drawing object ${(object as any).externalId || 'unknown'}:`, error)
				console.error(`[GBufferPass] Error details:`, {
					objectId: (object as any).externalId || 'unknown',
					hasMesh: !!object.mesh,
					meshReady: object.isMeshReady(),
	...
	public render(): void {
		...
		this.renderExternalObjects(); // Render external objects (cars, models)
