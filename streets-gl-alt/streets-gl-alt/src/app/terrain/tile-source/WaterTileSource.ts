	...
	private async fetchTile(x: number, y: number, zoom: number, scale: number): Promise<PBFPolygon[]> {
		...
		try {
			...
			if (response.status !== 200) {
				// Silently return empty polygons for 404s and other non-200 statuses
				// This is expected for areas without water data or CORS issues
				// Don't log warnings - these are expected and handled gracefully
				return polygons;
			}
			...
			// Handle network errors, CORS issues, and other fetch failures gracefully
			// Return empty polygons instead of throwing - this allows the map to continue rendering
			// Don't log errors - CORS and network failures are expected and handled gracefully
			// Return empty polygons to allow rendering to continue
		}
