...
export function mapIqCloudDensity(
  ...
  const smooth = cutoff + 0.12
  const t = Math.max(0, Math.min(1, (d - cutoff) / Math.max(0.0001, smooth - cutoff)))
...
export function estimateIqRaymarchAlpha(
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (den > 0.01) {
      const a = den * 0.42 * mix(0.6, 1, dayFactor)
      sumA += a * (1 - sumA)
    }
