# Verify v2.5 backup on both drives

Write-Host "v2.5 Backup Verification:" -ForegroundColor Cyan
Write-Host ""

$dPath = "D:\3d-viever-backup\v2.5"
$fPath = "F:\3d-viever-backup\v2.5"
...
if (Test-Path $fPath) {
    ...
    if (Test-Path $dPath) {
        ...
        if ($diff -eq 0) {
            Write-Host ""
            Write-Host "✅ D and F drives are in sync!" -ForegroundColor Green
        } else {
            Write-Host ""
