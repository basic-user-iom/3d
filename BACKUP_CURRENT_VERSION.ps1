# Backup Current Version Script
# Creates a backup following the existing version naming convention (v2.3, v2.4, etc.)
# Process: 
#   1. Backup to D:\3d-viever-backup (local backup on D drive)
#   2. Copy from D:\3d-viever-backup to F:\3d-viever-backup (external drive backup)

$localBackupDir = "D:\3d-viever-backup"      # Step 1: Local backup on D drive
$externalBackupDir = "F:\3d-viever-backup"   # Step 2: External backup on F drive
...
$tags = git tag -l | Where-Object { $_ -match '^v\d+\.\d+' } | Sort-Object {
    $version = $_ -replace '^v', ''
    $parts = $version -split '\.'
...
if ($tags) {
    ...
    $minor = [int]$parts[1]
    $nextMinor = $minor + 1
    $nextVersion = "v$major.$nextMinor"
    Write-Host "Latest version: $latestTag" -ForegroundColor Cyan
} else {
    Write-Host "No version tags found, starting with v2.3" -ForegroundColor Yellow
}
...
# Create version info file (matching existing backup format)
...
# Also create a text info file for easy reading
...
if (Test-Path $externalVersionBackupDir) {
    ...
    $response = Read-Host "Do you want to overwrite it? (y/n)"
    if ($response -ne 'y' -and $response -ne 'Y') {
        Write-Host "Skipping F drive copy." -ForegroundColor Yellow
    } else {
...
if (-not (Test-Path $externalVersionBackupDir)) {
    ...
    Write-Host "Version $nextVersion already exists on F drive, skipping copy." -ForegroundColor Yellow
}
